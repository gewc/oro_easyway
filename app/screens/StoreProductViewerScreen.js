...
const StoreProductViewerScreen = ({navigation, route}) => {
    const { pId, storeId } = route.params;
    ...
    const getStoreProducts = async () => {
        ...
        await axios.get(`/views/pviewer/${pId}`)
            .then((response) => {
                ...
                if(status !== "SUCCESS"){
                    ...
                    if(data.length < 1){
                        ...
                        handleMessage("There is no data to retrieve.")
                    }else{
                        setProductData(data)
                        setOldProductData(data)
                        handleMessage("")
        ...
        await axios.patch(`/views/${storeId}/${pId}`)
        .then((response) => {
            const result = response.data;
            const { message, status, data } = result;
            console.log("Get Products Result", message)
        })
        .catch( error => {
            console.log(error.message)
             handleMessage("An error occured. Check your network and try again!")
...
export default StoreProductViewerScreen
