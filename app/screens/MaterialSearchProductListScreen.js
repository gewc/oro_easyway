        ...
        <InnerContainer>
            <ProductContainer>
                ...
                <FlatList 
                    ...
                    renderItem={({item, index}) => <Item item={item} index={index} data={data} setVisible = {setVisibleUpdate} setProduct = {setProduct}/>}
...
const Item = ({item, index, data, setVisible, setProduct}) => (
    ...
    >
        ...
        <View style={{ width: '60%'}}>
            <Text style={{fontSize: 20,fontWeight: '700', marginLeft: 10,  color:primary  }}>{item.name.toUpperCase()}</Text>
            <Text style={{fontSize: 16,marginLeft: 10, marginBottom:5, marginTop:5, color:primary }}>{item.description.substring(0, 50)}</Text>
